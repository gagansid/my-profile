...
<div>
    ...
    <textarea id="excerpt" name="excerpt" rows="2" class="form-input mt-1 block w-full" required>{{ old('excerpt', $post->excerpt ?? '') }}</textarea>
    <x-input-error class="mt-2" :messages="$errors->get('excerpt')" />
</div>

<div>
    <x-input-label for="content" value="Konten (Markdown)" />
    <textarea id="content" name="content" rows="10" class="form-input mt-1 block w-full" required>{{ old('content', $post->content ?? '') }}</textarea>
...
<div>
    ...
    <x-text-input id="tags" name="tags" type="text" class="mt-1 block w-full" :value="old('tags', $post?->tags?->pluck('name')->join(', ') ?? '')" placeholder="Laravel, Tutorial" />
