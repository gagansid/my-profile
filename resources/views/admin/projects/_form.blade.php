...
<div>
    ...
    <select id="category_id" name="category_id" class="form-input mt-1 block w-full" required>
...
<div>
    ...
    <textarea id="description" name="description" rows="4" class="form-input mt-1 block w-full" required>{{ old('description', $project->description ?? '') }}</textarea>
    <x-input-error class="mt-2" :messages="$errors->get('description')" />
</div>

<div>
    <x-input-label for="demo_url" value="Demo URL (opsional)" />
    <x-text-input id="demo_url" name="demo_url" type="url" class="mt-1 block w-full" :value="old('demo_url', $project->demo_url ?? '')" />
    <x-input-error class="mt-2" :messages="$errors->get('demo_url')" />
</div>

<div>
    <x-input-label for="repo_url" value="Repo URL (opsional)" />
    <x-text-input id="repo_url" name="repo_url" type="url" class="mt-1 block w-full" :value="old('repo_url', $project->repo_url ?? '')" />
...
<div>
    ...
    @endif
    <input id="thumbnail" name="thumbnail" type="file" accept="image/*" class="form-input mt-1 block w-full">
...
<div>
    ...
    <x-text-input id="tags" name="tags" type="text" class="mt-1 block w-full" :value="old('tags', $project?->tags?->pluck('name')->join(', ') ?? '')" placeholder="Laravel, MySQL, PHP" />
    <x-input-error class="mt-2" :messages="$errors->get('tags')" />
</div>

<div>
    <x-input-label for="order" value="Urutan" />
    <x-text-input id="order" name="order" type="number" min="0" class="mt-1 block w-full" :value="old('order', $project->order ?? 0)" required />
    <x-input-error class="mt-2" :messages="$errors->get('order')" />
</div>

<div>
    <x-input-label for="status" value="Status" />
    <select id="status" name="status" class="form-input mt-1 block w-full" required>
